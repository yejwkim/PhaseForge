...
-- Apply via Supabase SQL Editor (paste + Run). Belongs with the web migrations
-- (0001–0003); kept here while the desktop branch is separate.
...
create table if not exists public.assessments (
  ...
  config_json  jsonb,            -- { questions, minutes, topics: [...] }
...
-- Demo code so the desktop app has something valid to test against right now.
-- Attaches to your first course. Remove once the instructor UI generates codes.
insert into public.assessments (course_id, title, code, status, config_json)
select
  id,
  'Demo Midterm',
  'DEMO-1234',
  'open',
  '{"questions": 15, "minutes": 60, "topics": ["Entropy", "Heat Engines", "Phase Transitions"]}'::jsonb
from public.courses
order by created_at
limit 1
